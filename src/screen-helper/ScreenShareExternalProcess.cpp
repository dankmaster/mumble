...
namespace {
	...
	RelayEndpoint materializeRelayEndpoint(const QJsonObject &plan,
		...
		if (scheme == QLatin1String("file")) {
			...
			const bool treatAsDirectory = relayUrl.endsWith(QLatin1Char('/')) || (originalInfo.exists() && originalInfo.isDir());
			...
			if (treatAsDirectory) {
				...
				const QString baseName =
					originalInfo.completeBaseName().trimmed().isEmpty() ? relayRoomID
		...
		if (scheme != QLatin1String("rtmp") && scheme != QLatin1String("rtmps")) {
			endpoint.errorMessage = QStringLiteral("Relay scheme %1 is advertised by the server but this helper build cannot publish it yet.")
	...
	bool selectCaptureSource(const ScreenShareExternalProcess::RuntimeSupport &support, QString *captureSource,
		...
		const QString configuredDisplay =
			qEnvironmentVariable("MUMBLE_SCREENSHARE_CAPTURE_DISPLAY",
								 qEnvironmentVariable("DISPLAY")).trimmed();
		const bool useTestPattern = forcedSource == QLatin1String("test-pattern")
			|| forcedSource == QLatin1String("lavfi") || envFlagEnabled("MUMBLE_SCREENSHARE_TEST_PATTERN");
		...
		if (useTestPattern) {
			...
			if (arguments) {
				...
				arguments->append(
					QStringLiteral("testsrc2=size=%1x%2:rate=%3").arg(width).arg(height).arg(fps));
			}
			return true;
		}
		...
		if (forcedSource == QLatin1String("x11") || forcedSource.isEmpty()) {
			...
			if (configuredDisplay.isEmpty()) {
				if (errorMessage) {
					*errorMessage =
						QStringLiteral("No DISPLAY is available for live desktop capture. Set MUMBLE_SCREENSHARE_TEST_PATTERN=1 for headless verification.");
				}
				return false;
			}
	...
	bool appendEncoderArguments(const ScreenShareExternalProcess::RuntimeSupport &support, const QJsonObject &plan,
		...
		const int maxRateKbps        = qMax(bitrateKbps, bitrateKbps + (bitrateKbps / 4));
		const int bufferSizeKbps     = qMax(maxRateKbps * 2, 5000);
		...
		auto appendRateControl = [&](const QString &encoder) {
			...
			arguments->append(QString::number(fps * 2));
		...
		switch (codec) {
			case MumbleProto::ScreenShareCodecH264:
				if (plannedBackend.contains(QLatin1String("nvenc")) && support.h264NvencAvailable) {
					...
					arguments->append(QStringLiteral("hq"));
				...
				if (plannedBackend.contains(QLatin1String("vaapi")) && support.h264VaapiAvailable && !renderNode.isEmpty()) {
	...
	ScreenShareExternalProcess::LaunchResult
		...
		if (process->waitForFinished(START_SETTLE_MSEC)) {
			...
			launch.errorMessage  = output.isEmpty() ? QStringLiteral("The external media process exited immediately.") : output;
			process->deleteLater();
			return launch;
		}
...
ScreenShareExternalProcess::LaunchResult ScreenShareExternalProcess::startPublish(const QJsonObject &plan, QObject *parent) {
	...
	if (!support.ffmpegAvailable) {
		...
		if (envFlagEnabled("MUMBLE_SCREENSHARE_ALLOW_STUB")) {
			...
			launch.usedStub      = true;
			launch.executionMode = QStringLiteral("stub");
			launch.warnings.append(QStringLiteral("ffmpeg is unavailable; using helper stub mode because MUMBLE_SCREENSHARE_ALLOW_STUB=1."));
		}
		return launch;
	}
	...
	for (const QString &candidateBackend : candidateBackends) {
		...
		if (!appendEncoderArguments(support, attemptPlan, &selectedEncoder, &attemptWarnings, &arguments, &encoderError)) {
		...
		if (candidateBackend.trimmed().toLower() != plannedBackend.trimmed().toLower()) {
			launch.warnings.append(
				QStringLiteral("Fell back from %1 to %2 during helper startup.")
					.arg(plannedBackend, candidateBackend));
		}
		return launch;
	}

	launch.errorMessage = lastError.isEmpty() ? QStringLiteral("No external ffmpeg encoder backend could be started.")
											  : lastError;
	if (envFlagEnabled("MUMBLE_SCREENSHARE_ALLOW_STUB")) {
		launch.started       = true;
		launch.usedStub      = true;
...
ScreenShareExternalProcess::LaunchResult ScreenShareExternalProcess::startView(const QJsonObject &plan, QObject *parent) {
	...
	if (headlessView ? !support.ffmpegAvailable : !support.ffplayAvailable) {
		...
		if (envFlagEnabled("MUMBLE_SCREENSHARE_ALLOW_STUB")) {
			...
			launch.executionMode = QStringLiteral("stub");
			launch.warnings.append(
				headlessView ? QStringLiteral("ffmpeg is unavailable for headless viewer mode; using helper stub mode because MUMBLE_SCREENSHARE_ALLOW_STUB=1.")
							 : QStringLiteral("ffplay is unavailable; using helper stub mode because MUMBLE_SCREENSHARE_ALLOW_STUB=1."));
		}
		return launch;
	}
	...
	launch.endpointUrl     = endpoint.localFilePath.isEmpty() ? endpoint.endpointUrl : QUrl::fromLocalFile(endpoint.localFilePath).toString();
